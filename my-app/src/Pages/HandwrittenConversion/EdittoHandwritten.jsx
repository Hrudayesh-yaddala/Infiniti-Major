...
const EdittoHandwritten = () => {
  ...
  const[textfileinput,setTextfileinput]=useState(false);
    ...
    const downloadImage = () => {
      ...
      // You can create a Blob object and create a URL for it to download
      const blob = new Blob([resdata]);
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'handwritten_image.png');
    ...
    const TranscribeFile = async () => {
        try {
          ...
          if (inputtext.length==0) {
            toast.error("Please Provide the text to continue");
            setDisplayResult(false);
            return;
          }
        ...
        {/* <h1>Feature is under progress...</h1> */}
           ...
           {/* </div> */}
                     ...
                     {/* <div className=' w-52 bg-gray-400 rounded-full gap-x-7 flex items-center justify-center p-2'>
                      ...
                      {/* </div> */}
                        ...
                        <div className="h-72 w-96  transition duration-500">
                          <textarea className=' w-full h-full border border-black rounded-lg p-2' placeholder=' Place your text here .....' onChange={InputTexthandlechange} />
                          </div>
            ...
            <div className=' flex items-center justify-center rounded-lg bg-slate-300 w-full h-72 mt-4'>
              ...
              <iframe src={resdata} className='w-full h-72  rounded-lg' />
              }
            </div>
            <div className='flex self-end space-x-3 mr-4'>
            <MdOutlineDownload onClick={downloadImage} className='cursor-pointer text-black p-2 rounded-lg bg-gray-500 text-4xl' />
...
export default EdittoHandwritten;
